...
```{r}
emo_glla <- emo %>%
    select(SID, Full_Date, all_beeps, trait, responses2) %>%
  mutate(dist_name = ifelse(trait == "happy", 1, 0),
         trait2 = ifelse(trait == "happy", 1, 2),
         SID2 = as.numeric(SID),
         SID3 = ifelse(trait2 == 1, SID2, SID2 + 100)) %>%
  drop_na() %>%
  group_by(SID, Full_Date) %>%
  filter(n() == 2) %>%
  group_by(SID) %>%
  filter(sum(!is.na(responses2))/2 > 40) %>%
  mutate(all_beeps = all_beeps - min(all_beeps) + 1) %>%
  ungroup() %>%
  arrange(SID, all_beeps, trait) 

emo_deriv <- rties::dataPrep(basedata = emo_glla
                , dyadId = "SID2"
                , personId = "SID3"
                , obs_name = "responses2"
                , time_name = "all_beeps"
...
compare <- rties::indivCloCompare(derivData = derivs$data)

plots <- rties::indivCloPlots(
  derivData=derivs$data
  , whichModel="coupled"
  , idConvention=100
  , plot_obs_name="obs_deTrend"
  , printPlots=F
  )
...
lpaData <- rties::inspectProfiles(
  whichModel="clo"
  , prepData=emo_deriv
  , paramEst=clo$params %>% drop_na()
  , n_profiles=3
  , minMax = c(.1, .9)
  , dist0name="happy"
  , dist1name="purposeful"
  )

fullData <- rties::makeFullData(
  basedata=emo_glla %>% select(-Full_Date) %>% arrange(SID, all_beeps, trait)
  , dyadId = "SID2"
  , personId = "SID3"
  , dist_name="dist_name"
  , lpaData=lpaData
...
profilePlots <- rties::plotDataByProfile(
  prepData=emo_deriv
  , fullData=fullData
  , n_profiles= 2
  , dist0name="happy"
