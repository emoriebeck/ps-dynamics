...
print(jobid)
args <- read.table("scripts/cluster/args/q2-mlm-pred.txt"
...
q2_pred_fun <- function(study, pname, pitem, sitem){
  ...
  load(sprintf("05-results/02-q2/03-mlm-uncentered/01-models/%s-%s-%s-%s.RData", study, pname, pitem, sitem))
  ...
  save(fx, rx, file = sprintf("05-results/02-q2/03-mlm-uncentered/02-summary/%s-%s-%s-%s.RData", study, pname, pitem, sitem))
  ...
  rx_draws <- draws %>% 
    ...
    mutate(pdeltavalue__p_value = pdeltavalue__p_value + b_pdeltavalue_p_value
  ...
  save(fx_draws, rx_draws
       , file = sprintf("05-results/02-q2/03-mlm-uncentered/05-draws/%s-%s-%s-%s.RData", study, pname, pitem, sitem))
  ...
  equil <- equil %>% 
    ...
    unnest(data)
  save(equil, file = sprintf("05-results/02-q2/03-mlm-uncentered/03-equil/%s-%s-%s-%s.RData", study, pname, pitem, sitem))
  ...
  # frame <- crossing(SID = unique(m$data$SID), p_value = seq(-5,5,.25), s_value = seq(-5,5,.25))
  rx_pred <- bind_cols(
    bind_rows(frame, frame)
  ...
  save(fx_pred, rx_pred
       , file = sprintf("05-results/02-q2/03-mlm-uncentered/04-predictions/%s-%s-%s-%s.RData", study, pname, pitem, sitem))
